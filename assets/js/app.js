...
function renderPapersGrid() {
    ...
    const papersToShow = filteredPapers.length > 0 ? filteredPapers : visiblePapers;
    ...
    if (papersToShow.length === 0) {
        ...
        if (resultsCount) {
            resultsCount.textContent = `Showing ${papersToShow.length} research paper${papersToShow.length !== 1 ? 's' : ''}`;
        }
    }
